...
// Cache Baileys version at startup — avoid repeated network calls on every session start
...
async function getBaileysVersion() {
    ...
    try {
        ...
        console.warn('⚠️ Could not fetch latest Baileys version, using fallback:', e.message);
        _cachedBaileysVersion = [2, 3000, 1015901307]; // safe fallback
    }
...
class SessionManager extends EventEmitter {
    ...
    async startSession(sessionId, config = null) {
        ...
        try {
            ...
            // Per-session connection watchdog — restart if stuck >90s in connecting
            let _watchdogTimer = null;
            const _startWatchdog = () => {
                if (_watchdogTimer) clearTimeout(_watchdogTimer);
                _watchdogTimer = setTimeout(() => {
                    this.log(`⚠️ Watchdog: session "${sessionId}" stuck connecting >90s, restarting.`, 'warn', sessionId);
                    ...
                    if (config.autoRestart !== false) {
                        ...
                        const baseDelay = Math.min(sessionObj.retryCount * 5000, 60000);
                        const jitter = Math.floor(Math.random() * 2000);
                        setTimeout(() => this.startSession(sessionId, config), baseDelay + jitter);
                    }
                }, 90_000);
            };
            ...
            sock.ev.on('connection.update', async (update) => {
                ...
                if (qr) {
                    try {
                        ...
                        _clearWatchdog(); // QR shown — not stuck
                ...
                if (connection === 'connecting') {
                    ...
                    _startWatchdog(); // Begin watchdog on connecting
                    this.emit('session_update', { id: sessionId, status: 'connecting' });
                }

                if (connection === 'open') {
                    _clearWatchdog(); // Connected — cancel watchdog
                ...
                if (connection === 'close') {
                    ...
                    if (statusCode === DisconnectReason.loggedOut || statusCode === 401) {
                        ...
                        this.log(`Reconnecting session "${sessionId}" in ${(waitMs/1000).toFixed(1)}s... (attempt ${sessionObj.retryCount})`, 'info', sessionId);
                        await delay(waitMs);
